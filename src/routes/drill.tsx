...
function DrillPage() {
  ...
  return <Page narrow><PageTitle eyebrow="Build your session" title="Custom Drill" subtitle="Choose what to focus on. You can change any setting before starting." />
    ...
    <div className="sticky bottom-21 z-20 mt-6 rounded-2xl border border-border bg-background/90 p-3 shadow-float backdrop-blur-xl md:bottom-4"><Button size="lg" className="w-full" disabled={selected.length === 0} onClick={() => navigate({ to: "/question", search: { source: "custom", material: selected[0] ?? "tiu-4", count, difficulty: "All", status: "All", challenge: false } })}><SlidersHorizontal /> Start Drill · {count} Questions</Button></div>
  </Page>;
}
